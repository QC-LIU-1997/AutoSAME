...
from .modeling import ImageEncoderViT, ImageEncoderViTPP, MaskDecoder, PromptEncoder,Same, TwoWayTransformer
from torch.nn import functional as F

def build_same_vit_h(args, checkpoint=None):
    return _build_same(
        args,
...
build_same = build_same_vit_h

def build_same_vit_b(args, checkpoint=None):
    return _build_same(
        args,
...
same_model_registry = {
    "default": build_same_vit_h,
    "vit_b": build_same_vit_b,
}
...
def _build_same(
    ...
    fcba_flag = args.fcba_flag
    bott_flag = args.bott_flag
    ...
    if checkpoint is not None:
        ...
        try:
            same.load_state_dict(state_dict)
        except:
            new_state_dict = load_from2(same, state_dict, image_size, patch_size)
            same.load_state_dict(new_state_dict)
    return same
